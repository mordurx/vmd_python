...
for frame in range(traj1.num_frames()):
    new_receptor='docking_megadock/poses/pdb_'+str(frame)+'.pdb'
    pdb=traj1.get_pdb_trajectory("docking_megadock/poses/pdb_ori",frame,frame)
    ...
    decoy=subprocess.call("decoygen docking_megadock/poses/snx_frame_"+str(frame)+".pdb "+ ligand+" guido.out 1",shell=True)
    cols=["ELEC "]
    df = pd.read_csv("guido.csv",usecols=cols)
    x= df.sort_values('ELEC ',ascending=True,axis=0,na_position ='first')
    #df.head()
    print (x['ELEC '].tolist()[0])
    score_vec.append(x['ELEC '].tolist()[0])
    time.append(frame)
    #os.remove(pdb)
...
#plt.show()
traj1.close()
plt.savefig("vsd3_elect_time.png", dpi=900) 
